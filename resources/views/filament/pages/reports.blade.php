<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-lg font-semibold mb-4">{{ __('Top Vehicles by Bookings') }}</h3>
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">{{ __('Vehicle') }}</th>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">{{ __('Bookings') }}</th>
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">{{ __('Daily Rate') }}</th>
                        </tr>
                    </thead>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-lg font-semibold mb-4">{{ __('Top Customers') }}</h3>
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold mb-4">{{ __('Booking Status Distribution') }}</h3>
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <div class="rounded-lg p-4 {{ $colors }}">
                    <div class="text-center">
                        ...
                        <div class="text-sm capitalize">{{ __(\Illuminate\Support\Str::ucfirst($status)) }}</div>
                    </div>
                </div>
                @endforeach
            </div>
